...
from .network import TripletLoss, SetNet_angle
...
class Model:
    def __init__(self,
        ...
        if self.model_name=='GaitSet':
            self.encoder = SetNet_angle(self.hidden_dim).float()
        elif self.model_name=='GaitSet_lstm':
            self.encoder = SetNet_lstm(self.hidden_dim).float()
            ...
            def select_frame(index):
                ...
                if self.sample_type == 'random':
                    if len(frame_set) >= 30:
                        ...
                        s_frame_id_list = np.random.choice(frame_set, size=(self.frame_num - len(frame_set)), replace=True).tolist()
    ...
    def fit(self):
        ...
        for seq, view, seq_type, label, batch_frame in train_loader:
            ...
            feature, label_prob,angle_prob = self.encoder(*seq, batch_frame)
    ...
    def transform(self, flag, batch_size=1):
        ...
        for i, x in enumerate(data_loader):
            ...
            feature, _,view_prob = self.encoder(*seq, batch_frame)
