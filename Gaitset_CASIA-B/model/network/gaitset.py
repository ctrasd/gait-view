...
class SetNet_angle(nn.Module):
    def __init__(self, hidden_dim):
        super(SetNet_angle, self).__init__()
        self.hidden_dim = hidden_dim
        self.batch_frame = None
        ...
        self.gem=GeM()
        self.cls=nn.Linear(in_features=128*2, out_features=11)
        
        self.trans_view=nn.ParameterList([
            nn.Parameter(
                nn.init.xavier_uniform_(
                    torch.zeros(62, 128, hidden_dim)))]*11)

        self.bin_num = [1, 2, 4, 8, 16]
        self.fc_bin = nn.ParameterList([
            nn.Parameter(
                nn.init.xavier_uniform_(
                    torch.zeros(62, 128, hidden_dim)))])
    ...
    def forward(self, silho, batch_frame=None):
        ...
        for num_bin in self.bin_num:
            ...
            z = z.mean(3) + z.max(3)[0]
            feature.append(z)
        feature = torch.cat(feature, 2).permute(0, 2, 1).contiguous() #trans_view # n 62 128
        #feature = torch.cat(feature, 2).permute(2, 0, 1).contiguous() # 
        #print(feature.shape)
        #feature = feature.matmul(self.fc_bin[0])
        #feature = feature.permute(1, 0, 2).contiguous()
        #print(self.trans_view[0].shape)
        #print(self.fc_bin[0].shape)
